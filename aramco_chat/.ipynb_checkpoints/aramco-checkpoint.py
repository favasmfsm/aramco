...
def format_chat_history(history):
    return "\n".join(
        f"{'You' if msg['role'] == 'user' else 'Assistant'}: {msg['content']}" for msg in history
    )

def get_response_from_llm(prompt):
    return client.responses.create(model="gpt-4.1-mini", input=prompt).output_text.strip()

def main():
    st.title("📊 Chat with Aramco Reports")
    ...
    if "chat_history" not in st.session_state:
        st.session_state.chat_history = [{"role": "assistant", "content": "How can I assist you with Aramco's yearly reports?"}]
    ...
    if user_input:
        ...
        # Refine search query
        search_prompt = [{"role": "system", "content": "You are a helpful assistant. Rewrite the user's query to be more specific."}]
        ...
        if st.session_state.year_filter:
            results = collection.query(query_embeddings=[query_embedding], n_results=5, where={"year": st.session_state.year_filter})
        else:
        ...
        if not docs:
            ...
            answer_prompt = [
                {"role": "system", "content": "You are an assistant answering questions based on annual report documents."},
